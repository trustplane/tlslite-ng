...
class AECDHKeyExchange(KeyExchange):
    ...
    def makeServerKeyExchange(self, sigHash=None):
        ...
        if self.group_id in [GroupName.ecdh_x25519, GroupName.ecdh_x448]:
            if self.group_id == GroupName.ecdh_x25519:
    ...
    def processClientKeyExchange(self, clientKeyExchange):
        ...
        if self.group_id in [GroupName.ecdh_x25519, GroupName.ecdh_x448]:
            ...
            if self.group_id == GroupName.ecdh_x25519:
                if len(ecdhYc) != 32 or ecdhYc[-1] >= 0x80:
            ...
            # check if shared secret is not all-zero
            summa = 0
            for i in sharedSecret:
                summa |= i
            if summa == 0:
                raise TLSIllegalParameterException("Invalid key share")
            return sharedSecret
    ...
    def processServerKeyExchange(self, srvPublicKey, serverKeyExchange):
        ...
        if serverKeyExchange.named_curve in [GroupName.ecdh_x25519,
                                             GroupName.ecdh_x448]:
            if serverKeyExchange.named_curve == GroupName.ecdh_x25519:
                generator = bytearray(X25519_G)
                fun = x25519
                ecdhXc = getRandomBytes(divceil(X25519_ORDER, 8))
                if len(serverKeyExchange.ecdh_Ys) != 32 or \
                        serverKeyExchange.ecdh_Ys[-1] >= 0x80:
