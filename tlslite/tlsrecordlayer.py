...
class TLSRecordLayer(object):
    ...
    def _getHandshakeFromBuffer(self):
        ...
        while len(self._handshakeBuffer) >= 4:
            parser = Parser(self._handshakeBuffer)
            # skip type
            parser.get(1)
            msgLength = parser.get(3)
            if parser.getRemainingLength() >= msgLength:
    ...
    def _getNextRecord(self):
        ...
        while True:
            ...
            #                    str(record.version), str(self.version))):
            #        yield result

            #Decrypt the record
            for result in self._decryptRecord(record.type, buf):
                if result in (0, 1):
                    yield result
                else: break
            ...
            #If it's an SSLv2 ClientHello, we can return it as well
                ...
                self._handshakeBuffer += buf
                self._handshakeRecord = record
